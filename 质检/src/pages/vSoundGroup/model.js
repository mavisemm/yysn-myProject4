...
export default {
        ...
        setup({ dispatch, history }) {
            return history.listen(({ pathname, search }) => {
                if (pathname === '/vSoundGroup') {
                    ...
                    dispatch({ type: 'querySoundDetector' });
                }
            });
        },
    },
    ...
    effects: {
        ...
        *getList({ payload = {} }, { call, put, select }) {
        ...
        // 获取听音器列表
        *querySoundDetector({ payload = {} }, { call, put, select }) {
            let params = {
                tenantId
            }
            const data = yield call(service.querySoundDetector, VtxUtil.handleTrim(params));
            let dataSource = [],
                total = 0,
                status = false;
            if (data && data.rc === 0) {
