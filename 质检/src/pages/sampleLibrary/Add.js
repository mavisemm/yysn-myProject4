...
class Add extends React.Component {
    ...
    findList = () => {
        const {turnType,startTime,endTime,machineId,receiverId,speed,machineNo,pointId,
        ...
        this.setState({
            ...
            if (selectedList.length) {
                ...
                for (let i = 0; i < selectedList.length; i++) {
                    recordIdList.push(selectedList[i].recordId)
                }
    ...
    dealEcharts = ()=>{
        ...
        if (this.echartsBoxDb) {
            ...
            if (myEchartsDb) {
                ...
                optionDb.xAxis[0].data = singleTotalArr[0].xArr || [];
        ...
        if (this.echartsBoxDensity) {
            ...
            if (myEchartsDensity) {
                ...
                optionDensity.xAxis[0].data = singleTotalArr[0].xArr || [];
    ...
    save = ()=>{
         const {turnType, machineId,receiverId,speed,detailDtoList,machineNo,
             pointId, detectorId, groupName, qualitySecondId, confirmFlag,
            qualityTemplateId} = this.state;
         if (groupName == '' || qualityTemplateId == '') {
             message.error('请检查输入框内容是否填写完整，品质等级是否选择！')
             return false
         }
         let params = {
             turnType,
    ...
    render(){
       ...
       const {machineList, qualityTemplateId,pointList,tableData,speedList,relation ,qualityList,querytemplateId,faultList,machineNo,groupName,
        qualityVisible,qualitySampleDetailDtoList,deviationList,confirmFlag } = this.state;
    return (
            <div>
                < Modal title = "样本库新增" 
                    ...
                    <div>
                        ...
                        <Select placeholder="请选择点位" style={{ width: 150, outline: 'none',marginLeft:10, }} onChange={this.pointChange.bind(this)}>
                            {
                    ...
                    {
                        querytemplateId == qualityTemplateId ?     <Select placeholder="故障类型" style={{ width: 120, outline: 'none',margin:"0 10px", }} onChange={this.faultChange.bind(this)}>
                            {
                                    ...
                                    <div key={index}>
                                        {
                                            (item.qualitySampleDetailDtoList || []).map((itemp,indexp)=>{
                                                return (
                                                    <div key={indexp}>样本参数组名称：{itemp.conditionName},偏离度阈值：{itemp.thresholdValue}</div>
                                                )
                                            })
                                        }
                                    </div>
