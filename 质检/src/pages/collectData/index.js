...
import SearchPage from '@src/pages/acomponents/waterSearch';
...
class standardStore extends React.Component{
        ...
        let that = this;
         setTimeout(() => {
              ...
              if (that.echartsBoxDb) {
                  ...
                  if (myEchartsDb) {
                    const {distVisible} = that.state;
                    let optionDb = distVisible ? JSON.parse(JSON.stringify(echartsOption.optionDist)) : JSON.parse(JSON.stringify(echartsOption.optionDb));
                      optionDb.xAxis[0].data = XARR || [];
                      optionDb.series = distVisible ? totaldistArr : finallydbArr;
             ...
             if (type == 1) {
                ...
                // 距离频率
                   if (this.echartsBoxFullScreen) {
                       ...
                       if (myEchartsFullScreen) {
                           let optionDb = JSON.parse(JSON.stringify(echartsOption.optionDist));
                           optionDb.xAxis[0].data = XARR || [];
                           optionDb.series = finallydistArr || [];
                           myEchartsFullScreen.setOption(optionDb)
                           //  myEchartsFullScreen.on('finished', () => {
                           //      if (myEchartsFullScreen) {
                           //          myEchartsFullScreen.resize()
    ...
    // 计算
    compareCalulate =()=>{
        const {detailDtoList,tableData,start,end} = this.state;
        if (start == '' || end == '') {
        ...
        let freqs = responseList.freqs;
        let dataArr = responseList.responseList;
        for (let i = 0; i < dataArr.length; i++) {
            let total = 0;
            for (let j = 0; j < dataArr[i].dbArray.length; j++) {
                let temp = dataArr[i].dbArray[j];
            
                if (freqs[j] > start && freqs[j] < end) {
                    total += temp; 
                }
            }
            dataArr[i] = {
        ...
        for(let i = 0;i<tableData.length;i++){
            ...
            for(let j = 0;j<dataArr.length;j++){
                if (tableData[i].id == dataArr[j].recordId){
                    arr.push({
                      ...temp,
                      total:dataArr[j].total
                        
                    })
                }
            }
        }
        let tempArr = this.sortArrayByTotalDescending(arr);
        for(let j =0;j<tempArr.length;j++){
            tempArr[j] = {
                ...tempArr[j],
                sort:j+1,
                detectTime:moment(tempArr[j].detectTime).format('YYYY-MM-DD HH:mm:ss')
            }
        }
        this.setState({
            calculateVisible: true,
            compareTable: tempArr
        })
    ...
    return (
            <div className={styles.body}>
                ...
                <div>
                    <Input addonBefore="开始频率：" addonAfter='Hz'  style={{width:'250px',}} placeholder="请输入:" value={start} name='start'
                     onChange={this.inputChange.bind(this)} />
                    <Input addonBefore="结束频率：" addonAfter='Hz' style={{width:'250px',marginLeft:"10px"}} placeholder="请输入:" value={end} name='end'
                     onChange={this.inputChange.bind(this)} />
                     <Button type='primary' style={{marginLeft:"10px"}} onClick={()=>this.compareCalulate()}>开始计算</Button>
                </div>
                {/* 能量密度 */}
               <div className={styles.frequencyWidth}>
                        ...
                        <Button type='primary' style={{marginLeft:10}} onClick={()=>{
                            this.lookFullScreen(3)
                        }}>全屏距离曲线</Button>
                          <Switch style={{marginLeft:10}} checkedChildren="距离密度" unCheckedChildren="能量密度" defaultChecked={true} onChange={this.changeBtn.bind(this)}/>
                         <span style={{color:'red',fontSize:16}}>提示：数据量大时可以通过点击具体频率查看所选数据对应的能量、密度 </span>
                        </div>
    
                <div className={styles.frequencyWidth}>
                    {
                        this.state.calculateVisible &&

                        <Table {...tableStyle}  rowKey={record => record.id} columns={columns1} dataSource={compareTable} />
                    }
                    <BtnWrap>
