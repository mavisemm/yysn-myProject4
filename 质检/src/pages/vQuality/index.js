...
import { Modal, Button, message, Input, Select, Icon,Table,Checkbox,Tabs,Popconfirm } from 'antd';
const { Option } = Select;
...
const namespace = 'vQuality';
import SideBar from '@src/pages/sideBar';
const CheckboxGroup = Checkbox.Group;
class vQuality extends React.Component {
    state = {
        loading: false,
        visible:false,
        tableData:[],
        ...
        id:'',
        checkedList:[],
        plainOptions:['曲线','周期声音','标准声音','典型样本','突发声音','分区声音','点位周期声音'],
        deleteId:""
    }
    componentDidMount() {
        this.getList();
...
// plainOptions:['曲线','周期声音','标准声音','典型样本','突发声音','分区声音','点位周期声音']
                if (res.ret) {
                    ...
                    if (cycleSameStatus == 1){
                        checkedList.push('点位周期声音')
                    }
                    this.setState({
                        checkedList,
                        id,
    ...
    render(){
        const columns = [
            ...
            
            {
                ...
                render: (text, record) => (
                    <span>
                        <Popconfirm placement="topLeft" title='谨慎操作！！确认删除吗？' onConfirm={this.delete.bind(this)} okText="确定" cancelText="取消">
