 ...
 * 听音参数设置
 * author : vtx shh
 * createTime : 2021-08-04 11:25:00
 */
...
const namespace = 'vParams';
function vParams({ dispatch, vParams, loading }) {
    const {
        searchParams,
        machineRoomList,
        machineList,
    ...
    const columns = [
        ['采音时间(s)', 'listenTime'],
        ['开始频率(Hz)', 'startFrequency'],
        ['结束频率(Hz)', 'endFrequency'],
        ['分段数量', 'frequencyCount'],
        [
            '操作',
            'action',
...
export default connect(({ vParams, loading }) => ({ vParams, loading }))(
    vParams,
);
