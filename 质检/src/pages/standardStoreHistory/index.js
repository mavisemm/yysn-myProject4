...
class standardStoreHistory extends React.Component{
    constructor(props) {
        super(props);
        this.state = {
            loading: false,
    ...
    componentDidMount(){
        ...
        const {machineList,pointList,qualityList} = standardStoreHistory;
        this.setState({
            machineList,
            pointList,
            qualityList
    ...
    compareData=(record,index)=>{
        this.setState({
            weidu:record.type,
            id:record.id,
            compareDataVisible:true,
        })
    }
    // 设为使用中
    setUse = (record,index)=>{
        ...
        if(type == 0){
            service.setUseFrequency(VtxUtil.handleTrim(params)).then(res => {
                if (res.rc == 0) {
        ...
        if (type == 1) {
            service.setUseCycle(VtxUtil.handleTrim(params)).then(res => {
                if (res.rc == 0) {
                    message.success('设置成功')
                    this.getList(1);
                } else {
        ...
        if (type == 2) {
            service.setUseDeviation(VtxUtil.handleTrim(params)).then(res => {
                if (res.rc == 0) {
                    message.success('设置成功')
                    this.getList(1);
                } else {
        ...
        if (type == 3) {
            service.setUseSudden(VtxUtil.handleTrim(params)).then(res => {
                if (res.rc == 0) {
                    message.success('设置成功')
                    this.getList(1);
                } else {
        ...
        if (type == 4) {
            service.setUsePartition(VtxUtil.handleTrim(params)).then(res => {
                if (res.rc == 0) {
                    message.success('设置成功')
                    this.getList(1);
                } else {
        ...
        if (type == 5) {
            service.setUsePointCycle(VtxUtil.handleTrim(params)).then(res => {
                if (res.rc == 0) {
                    message.success('设置成功')
                    this.getList(1);
                } else {
    ...
    delete = (record)=>{
        ...
        if(type == 1){
            // 周期
            service.deleteCycle([id]).then(res => {
                if (res.rc == 0) {
                    message.success('删除成功')
                    this.getList(1);
                } else {
        ...
        if(type == 2){
            // 标准声音
            service.deleteDeviation([id]).then(res => {
                if (res.rc == 0) {
                    message.success('删除成功')
                    this.getList(1);
                } else {
        ...
        if(type == 3){
            // 突发声音
            service.deleteSudden([id]).then(res => {
                if (res.rc == 0) {
                    message.success('删除成功')
                    this.getList(1);
                } else {
        ...
        if(type == 4){
            // 分区声音
            service.deletePartition([id]).then(res => {
                if (res.rc == 0) {
                    message.success('删除成功')
                    this.getList(1);
                } else {
        ...
        if (type == 5) {
            // 点位周期声音声音
            service.deletePointCycle([id]).then(res => {
                if (res.rc == 0) {
                    message.success('删除成功')
                    this.getList(1);
                } else {
    ...
    copyData = (record) => {
        this.setState({
            machineId1:record.machineId,
            pointId1:record.pointId,
            groupId: record.id,
            type:record.type,
            copyVisible: true,
        })
    ...
    confirmCopy = () => {
        const {id,type,groupId,machineId1,pointId1,name} = this.state;
        let params = {
            id:groupId,
        ...
        if(type == 0){
            // 曲线
            service.copyFrequency({...params}).then(res => {
                if (res.rc == 0) {
        ...
        if(type == 1){
            // 周期
            service.copyCycle({...params}).then(res => {
                if (res.rc == 0) {
                    message.success('复制成功')
                    this.getList(1);
                } else {
        ...
        if(type == 2){
            // 标准声音
            service.copyDeviation({...params}).then(res => {
                if (res.rc == 0) {
                    message.success('复制成功')
                    this.getList(1);
                } else {
        ...
        if(type == 3){
            // 突发声音
            service.copySudden({...params}).then(res => {
                if (res.rc == 0) {
                    message.success('复制成功')
                    this.getList(1);
                } else {
        ...
        if(type == 4){
            // 分区声音
            service.copyPartition({...params}).then(res => {
                if (res.rc == 0) {
                    message.success('复制成功')
                    this.getList(1);
                } else {
        ...
        if (type == 5) {
            // 点位周期声音声音
            service.copyPointCycle({...params}).then(res => {
                if (res.rc == 0) {
                    message.success('复制成功')
                    this.getList(1);
                } else {
            ...
            //   < Button onClick={()=>this.compareData(record,index)}> 比对 </Button>
        ...
        const columns = [
            ...
            },
            {
                ...
                render: (text, record,index) => (
                    <span>
                        {
                            record.type == 0 ? '标准曲线' : (record.type == 1 ? '标准周期' : (record.type == 2 ? '标准声音' : (record.type == 3 ? '突发声音' : (record.type == 4 ? '分区声音': '点位周期声音')
                                )))
                        }
                    </span>
                ),
            ...
            }, 
            {
                ...
                render: (text, record,index) => (
                    ...
                    < span className = "ant-divider" />
                    {/* <Popconfirm placement="topLeft" title='确认复制此标准库吗？' onConfirm={this.copyData.bind(this)} okText="确定" cancelText="取消">
                     < Button type='danger' onClick={()=>this.deleteMode(record)}> 复制 </Button>
                    </Popconfirm> */}
                
                     < Button type='danger' onClick={()=>this.copyData(record)}> 复制 </Button>
                    < span className = "ant-divider" />
    ...
    return (
        <Page title='历史标准库管理'>
            ...
            <div className={styles.body} style={{width:'90%'}}>
                ...
                {this.state.singleDataVisible && <Edit id={this.state.id} parent={this} weidu={this.state.weidu}></Edit>}
                {this.state.compareDataVisible && <CompareEdit id={this.state.id} parent={this} weidu={this.state.weidu}></CompareEdit>}
                ...
                < Modal title = "复制标准库" 
                    ...
                    width="40%"
                    >
                         <Input addonBefore="标准库名称："  style={{width:'500px'}} value={name} name='name'
                                onChange={this.inputChange.bind(this)} />
                        <div style={{margin:'10px 0'}}>
                            <Select value={machineId1} placeholder="请选择机型" style={{width:300}} onChange={this.chooseMachine1.bind(this)} 
                                   filterOption={(input, option) => {
                                    // 确保 children 是字符串，并且调用 toLowerCase()
                                    const childrenStr = option.props.children.toString().toLowerCase();
                                    return childrenStr.indexOf(input.toLowerCase()) >= 0;
                        ...
                        <div>
                            <Select value={pointId1} placeholder="请选择点位" style={{ width: 300 }} onChange={this.pointChange1.bind(this)} 
                                   filterOption={(input, option) => {
                                    // 确保 children 是字符串，并且调用 toLowerCase()
                                    const childrenStr = option.props.children.toString().toLowerCase();
                                    return childrenStr.indexOf(input.toLowerCase()) >= 0;
...
export default standardStoreHistory;
