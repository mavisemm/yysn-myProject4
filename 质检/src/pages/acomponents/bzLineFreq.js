...
class bzFrequency extends React.Component {
    ...
    componentDidMount() {
        const {multiAbsFreqStandardGroupDtoList} = this.props;
        this.setState({
            multiAbsFreqStandardGroupDtoList,
        }, () => {
        })
     ...
     componentWillReceiveProps(newProps) {
        const {multiAbsFreqStandardGroupDtoList} = newProps;
        this.setState({
            multiAbsFreqStandardGroupDtoList,
        },()=>{
        })
   ...
   typeChange1=(index,indexp,e)=>{
    ...
    // 频段确认弹窗
    freqOk = ()=>{
        const {name,multiAbsFreqStandardGroupDtoList,detailDtoList ,
        currentIndex,btnType,qualityId,qualityName,color} = this.state;
        if (qualityId == '') {
        ...
        if(btnType == 'add'){
            let arr = [...multiAbsFreqStandardGroupDtoList];
            let paramsArr = [{
                qualityName,
                qualityId,
                color,
            ...
            this.setState({
                multiAbsFreqStandardGroupDtoList:arr.concat(paramsArr),
                freqVisible:false,
            })
            // console.log(arr.concat(paramsArr),'提交参数')
            // return false;
            this.props.parent.getLineSet(this, arr.concat(paramsArr));
         }else{
            // 编辑
            let arr = [...multiAbsFreqStandardGroupDtoList];
            arr[currentIndex]={
            ...
            this.setState({
                multiAbsFreqStandardGroupDtoList:arr,
                freqVisible:false,
            })
            this.props.parent.getLineSet(this, arr);
         }  
    ...
    // =================table表格删除
    delete = () => {
        const {multiAbsFreqStandardGroupDtoList,deleteIndex} = this.state;
        let arr = [...multiAbsFreqStandardGroupDtoList]
    ...
    render(){
        ...
        const {id ,multiAbsFreqStandardGroupDtoList,detailDtoList,contentList,typeList,color ,qualityId,qualityName,btnType } = this.state;
        return (
            <div style={{width:'50%'}}>
                <BtnWrap>
                    <Button type = "primary" onClick={()=>this.add()}> 新增直线品质等级 </Button>
                </BtnWrap>
                {
                    (multiAbsFreqStandardGroupDtoList && multiAbsFreqStandardGroupDtoList.length != 0) &&  <Table rowKey={record => record.id}  columns={column} dataSource={multiAbsFreqStandardGroupDtoList || []} pagination={false}/>
                }
