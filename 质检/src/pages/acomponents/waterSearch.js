...
class SearchPage extends React.Component {
    ...
    getList = ()=>{
        const {cycleType,startTime,endTime,machineId,receiverId,speed,machineNo,pointId,qualityId,
            detectorId,machineName,pointName,receiverIdList} = this.state;
        let params = {
            cycleType:0,
            endTime,
            machineId,
    ...
    render(){
        const {
            ...
            pointList,
            speedList,
           machineNo, pointName, machineName, speed, pointFileUrl
        } = this.state;
        return (
            <div>
                <div style={{marginBottom:"10px"}}>
                    ...
                    <Select mode="multiple" style={{ width: 300,marginLeft:10, outline: 'none' }} onChange={this.pointChange.bind(this)} 
                           filterOption={(input, option) => {
                            // 确保 children 是字符串，并且调用 toLowerCase()
                            const childrenStr = option.props.children.toString().toLowerCase();
                            return childrenStr.indexOf(input.toLowerCase()) >= 0;
