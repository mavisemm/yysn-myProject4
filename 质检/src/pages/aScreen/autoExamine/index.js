...
class voiceIndex extends React.Component {
    ...
    getEquipmentInfo = () => {
        ...
        // 死数据测试：定义为数组格式，每个元素包含label（标签）和value（值）
        const mockEquipmentInfo = [
            { label: '主轴转速', value: 's5500 转/分钟' },
            { label: '进给参数', value: 'F1.0' },
            { label: '刀具尺寸', value: '14.997 mm' },
            { label: '设备厂家', value: '开孚' },
        ];

        this.setState({
            // 存储数组格式的设备信息，而非固定对象
            equipmentInfoList: mockEquipmentInfo
        });
    }
    ...
    run() {
        ...
        const { machineId, groupType, detailDtoList, degree, degree2, speed, machineNo, receiverGroupId, listenTime, remark } = this.state;
        let detectorIdList = [];
        for (let i = 0; i < detailDtoList.length; i++) {
    ...
    render() {
        ...
        const { timeValue, djsVisible, machineName, groupType, machineId, machineGroupList,
            detectorList, receiverResponseList, staticsData, totalCount, wifiVisible, plcCount, remark,
            normalCount,
            ...
            <div className={styles.bodyScreen}>
                <div className={styles.headStyle}>
                    ...
                    <div className={styles.flex}>
                        {/* <Button type='primary' style={{marginRight:20}} onClick={()=>this.lookStatus()}>查询听音器状态</Button> */}
                        <img onClick={() => this.goPage()} src={require('@src/assets/admin.png')} className={styles.wifiicon} title='前往展示板'
                            alt='展示板' />
                ...
                <div className={styles.scanStyle}>
                    ...
                    <div className={styles.bdBg1}>
                        <div className={styles.equipmentTitle}>设备信息</div>
                        <div className={styles.equipmentInfoContainer}>
                            {/* 动态渲染设备信息，无数据时显示「未获取」 */}
                            {
                                (this.state.equipmentInfoList || []).length > 0 ?
                                    this.state.equipmentInfoList.map((item, index) => (
                                        <div className={styles.equipmentInfoItem} key={index}>
                                            <span className={styles.infoLabel}>{item.label}：</span>
                                            <span className={styles.infoValue}>{item.value || '未获取'}</span>
                                        </div>
                                    )) :
                                    <div className={styles.equipmentInfoItem}>
                                        <span className={styles.infoValue}>暂无设备信息</span>
                                    </div>
                            }
                        </div>
                    </div>
                    <div className={styles.zcUserconfirm}>
                        ...
                        <div className={styles.zcUserconfirmflex}>
                            <img src={require('@src/assets/voice/index3.png')} />
                            <Input className={styles.InputStylezc} placeholder='请输入备注' name='remark' onChange={this.inputChange.bind(this)} value={remark} />
                ...
                <div className={styles.echartsWrap}>
                    {/* 左侧刀具图片 */}
                    <img
                        src={require('@src/assets/刀具图片.jpg')}
                        alt="刀具图片"
                        className={styles.toolImg}
                    />
                    {/* 右侧ECharts容器（flex:1占满剩余宽度） */}
                    <div className={styles.echartsContainer}>
                        {
                            showResult && (resultArr || []).map((item, index) => {
                                return (
                                    <WholeBar
                                        receiverResponseList={item.receiverResponseList}
                                        recordDto={item.recordDto}
                                        parent={this}
                                        key={index}
                                    />
                                )
