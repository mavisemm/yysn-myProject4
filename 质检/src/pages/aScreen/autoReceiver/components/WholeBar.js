...
class WholeBar extends Component {
    ...
    componentDidMount() {
        ...
        this.setState({
            ...
            machineId,
            recordDto,
            cycleStatus: status.cycleStatus ? status.cycleStatus : '1',
            frequencyStatus: status.frequencyStatus ? status.frequencyStatus : '1',
            standardStatus: status.standardStatus ? status.standardStatus : '1',
            sampleStatus: status.sampleStatus ? status.sampleStatus : '1',
            partitionStatus: status.partitionStatus ? status.partitionStatus : '1',
        },()=>{
    ...
    dealEcharts = (receiverResponseList) => {
        for(let i = 0;i<receiverResponseList.length;i++){
            ...
            for (let j = 0; j < receiverResponseList[i].frequencyDtoList.length; j++) {
                ...
                if (receiverResponseList[i].standardFrequencyDtos) {
                    if (receiverResponseList[i].standardFrequencyDtos[j]) {
                        if (receiverResponseList[i]?.standardFrequencyDtos[j].db == 0) {
                            standardDbArr.push(undefined)
                        } else {
                            standardDbArr.push(receiverResponseList[i]?.standardFrequencyDtos[j].db.toFixed(2))
                        }
                        standardDensityArr.push(receiverResponseList[i]?.standardFrequencyDtos[j].density.toFixed(3))
                    }
            ...
            this.initEcharts(xArr, dbArr, densityArr, receiverResponseList[i].receiverId, standardDbArr, standardDensityArr)
            
        }
    }
    initEcharts = (xArr, dbArr, densityArr, id, standardDbArr, standardDensityArr) => {
        ...
        if (standardDbArr.length) {
            option.legend.data = ['密度', '能量', '标准密度', '标准能量'];
            option.series[2].data = standardDensityArr;
            option.series[3].data = standardDbArr;
        } else {
    ...
    saveEchart = ()=>{
        const {tenantId,listenTime,recordId,machineNo,machineId} = this.state;
        let params = {
            // tenantId,
            pojoList,
            recordId,
    ...
    lookSingle = (row)=>{
        ...
        this.setState({
            ...
            let standardFrequencyDtos = row.standardFrequencyDtos;
            ...
            if (standardFrequencyDtos){
                for (let j = 0; j < standardFrequencyDtos.length; j++) {
                    tempArr.push({
                        ...standardFrequencyDtos[j],
                        frequency:Math.sqrt(Number(standardFrequencyDtos[j].freq1) * Number(standardFrequencyDtos[j].freq2)).toFixed(0)
                    })
                    if (standardFrequencyDtos[j].db == 0) {
                        standardDbArr.push(undefined)
                    } else {
                        standardDbArr.push(standardFrequencyDtos[j].db.toFixed(2))
                    }
                    standardDensityArr.push(standardFrequencyDtos[j].density.toFixed(3))
                }
            }

            this.setState({
                standardFrequencyDtos: tempArr,
            })
